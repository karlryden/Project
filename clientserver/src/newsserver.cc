...
NewsServer::NewsServer(int port, bool use_disk): Server(port), db{use_disk ? (static_cast<Database*>(new DiskDatabase())) : (static_cast<Database*>(new MemoryDatabase()))}{
...
std::string NewsServer::handle_request(const std::shared_ptr<Connection>& conn){
    ...
    switch (comm){
        ...
        {
            std::cout << "Recieved request to list newsgroups" << std::endl;
            // Read the end-byte 
            conn->read();
        ...
        case(static_cast<int>(Protocol::COM_CREATE_NG)):{
            std::cout << "Recieved request to create newsgroup" << std::endl;
            // Read the par-string byte, read the string, read the end-byte
            conn->read();
            std::string title{readString(conn)};
            std::cout << "Recieved title: " << title << std::endl;
            conn->read();
            // Write the reply
        ...
        case (static_cast<int>(Protocol::COM_DELETE_NG)):{
            std::cout << "Recieved request to delete newsgroups" << std::endl;
        ...
        case(static_cast<int>(Protocol::COM_LIST_ART)):{
            std::cout << "Recieved request to list articles" << std::endl;
            // Read the par-n byte, the ID, and the end-byte
            conn->read();
            int id{readNumber(conn)};
            std::cout << "Recieved ng id: " << id << std::endl;
            conn->read();
        ...
        case(static_cast<int>(Protocol::COM_CREATE_ART)):{
            ...
            std::string author;
            std::string text;
            std::cout << "Recieved request to create article" << std::endl;
            // Read the NG-id
            conn->read();
            id = readNumber(conn);
            std::cout << "Ng id: " << id << std::endl;
            // read the title
            conn->read();
        ...
        case(static_cast<int>(Protocol::COM_DELETE_ART)):{
            int ng_id;
            int art_id;
            std::cout << "Recieved request to delete article" << std::endl;
            // read the ng id
            conn->read();
        ...
        case(static_cast<int>(Protocol::COM_GET_ART)):{
            ...
            int art_id;
            // read the ng id
            std::cout << "Recieved request to get article" << std::endl;
            conn->read();
            ng_id = readNumber(conn);
